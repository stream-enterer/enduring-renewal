...
public class Img64V2 {
   static final int maxWidth = 63;
   static final int maxHeight = 126;
   static final int maxPix = 7938;
   ...
   public static Texture fromString(String toParse) {
      ...
      if (width > 63) {
         ...
         for (int i = 0; i < numCols; i++) {
            ...
            if (c.equalsIgnoreCase("000")) {
               palette[i] = Colours.transparent;
            } else {
               palette[i] = Colours.from64(c);
            }
         }
         ...
         return height > 126 ? null : createTexture(width, height, palette, dataString, paletteBits);
      }
   }
